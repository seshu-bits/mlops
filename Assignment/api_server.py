...
prediction_count = Counter(
    "predictions_total", "Total number of predictions made", ["prediction_result", "model_name"]
)
...
prediction_latency = Histogram(
    "prediction_duration_seconds", "Prediction processing time in seconds", ["endpoint"]
)
...
# Counter for errors
error_count = Counter("api_errors_total", "Total number of errors", ["error_type", "endpoint"])
...
def load_model(model_path: str = "artifacts/logistic_regression.pkl"):
    ...
    if scaler_path.exists():
        ...
        logger.warning(
            f"No scaler found at {scaler_path} - predictions may be incorrect without preprocessing!"
        )
        scaler = None
...
async def predict(patient: PatientData):
    ...
    try:
        ...
        if scaler is not None:
            numeric_cols = input_data.select_dtypes(
        ...
        logger.info(
            f"Prediction made: {prediction_result}, confidence: {confidence:.3f}, duration: {duration:.3f}s"
        )

        return PredictionResponse(
            prediction=int(prediction), confidence=confidence, model_name=model_name
        )
    ...
    try:
        ...
        for patient in batch.patients:
            ...
            prediction_count.labels(
                prediction_result=prediction_result, model_name=model_name
            ).inc()
            confidence_scores.labels(prediction_result=prediction_result).observe(confidence)
