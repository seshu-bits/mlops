...
def test_preprocessing():
    ...
    numeric_cols = test_input.select_dtypes(
    ...
    print(
        f"   ✓ Scaled data range: [{test_input_scaled.min().min():.2f}, {test_input_scaled.max().max():.2f}]"
    )
    ...
    print("\n6. Comparing results...")
    print(
        f"   Without scaling: pred={pred_no_scaling}, prob={prob_no_scaling[pred_no_scaling]:.4f}"
    )
    print(
        f"   With scaling:    pred={pred_with_scaling}, prob={prob_with_scaling[pred_with_scaling]:.4f}"
    )
